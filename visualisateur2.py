    ...
    def backward(self, batch_sz):
        ...
        for o in range(self.size_output):
            for i in range(self.size_layer):
                ...
                self.layerow[i][o] = adam.update(self.layerow[i][o], 0.9*dw)
        ...
        for o in range(self.size_output):
            for f in range(self.size_layer):
                for i in range(self.size_input):
                        ...
                        adam = Adam(learning_rate = self.LR)
                        self.layer1w[i][f] = adam.update(self.layer1w[i][f], 0.9*dw)
...
nn = ConvNeuralNet(256, 16, 3, 0.001)
nn.add_layer2(16)
nn.load_file2("nnmodel/40.104x256x16x16x4x0.001x2lu_ly2.txt")#41.16_256x32x16x3x0.001_ly2.txt")#56.77_256x32x4x0.001_1.txt")#48.4375_256x32x4x0.001_1.txt")
...
plt.figure()
plt.imshow(W3, cmap='inferno_r', aspect='auto')
plt.colorbar()
...
plt.colorbar()
plt.title('Weights of the ouput layer')
