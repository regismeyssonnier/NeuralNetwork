...
for X in range(10000):
	...
	for o in range(size_output):
		if o == (TEST_out[I]-1):
			...
			l += (0.0 - output[0][o]) *(0.0 - output[0][o])
	Loss.append(l/4)
...
for X in range(len(TEST)):
	...
	#del inputn[:]
	inputn = []
	inputn = test2inputn(TEST[X])
	print("l:"+str(len(inputn)))
	display_inputn(inputn)
	display_test(inputn, TEST_out[X])
	...
	h = []
	h = calc_output_sigmoid2(hidden[0], hiddenw[1], hiddenb[1], size_hidden, size_hidden)
	#print("output hid1 hid2")
	#print(h)
	hidden.append(h);
